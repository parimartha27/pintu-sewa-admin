...
export default function ManageReportContent() {
  ...
  const [selectedChat, setSelectedChat] = useState<ListChatResponse | null>(null)
  const [groupedMessages, setGroupedMessages] = useState<GroupedMessageResponse[]>([])

  // Fetch daftar chat
  useEffect(() => {
    const fetchChats = async () => {
      try {
        const response = await axios.get("https://pintu-sewa-backend.up.railway.app/api/chat/admin/get-roomchat")
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const response = await axios.get(`https://pintu-sewa-backend.up.railway.app/api/chat/view-roomchat/${selectedChat.id}`)
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await axios.post("https://pintu-sewa-backend.up.railway.app/api/chat/send-message", {
        ...
        <div className='flex flex-row w-full flex-1 overflow-hidden bg-white shadow-md rounded-xl'>
            ...
            {chatList.length === 0 ? (
              ...
              chatList.map((chat) => (
                <div
                  ...
                  className={`w-full flex flex-row items-center gap-4 p-4 cursor-pointer hover:bg-blue-50 border-b ${selectedChat?.id === chat.id ? "bg-blue-100" : ""}`}
                >
          ...
          <section className='w-2/3 flex flex-col justify-between border-l'>
            {selectedChat ? (
                ...
                <div className='flex-1 p-4 overflow-y-auto flex flex-col gap-4'>
                  {groupedMessages.length === 0 ? (
                    ...
                    groupedMessages.map((group) => (
                      <div key={group.date}>
                        <div className='text-center text-gray-500 text-sm mb-4'>{group.date}</div>
                        {group.messages.map((msg, i) => (
                          <div
                            key={i}
                            className={`flex mb-4 ${msg.sender_type === "shop" ? "justify-end" : "justify-start"}`}
                          >
                            <div className={`${msg.sender_type === "shop" ? "bg-blue-800 text-white" : "bg-blue-200 text-gray-900"} text-sm px-4 py-2 rounded-xl max-w-[60%]`}>
                              <p>{msg.message}</p>
              ...
              <div className='flex items-center justify-center h-full text-gray-400 text-lg'>Pilih chat untuk mulai melihat isi percakapan</div>
            )}
          </section>
        </div>
      </main>
