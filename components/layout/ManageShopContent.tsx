...
export default function ManageShopContent() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get<ShopApiResponse>(`https://pintu-sewa-backend.up.railway.app/api/admin/manage-shop/${currentPage}`)
  ...
  const filteredShops = shops.filter((shop) => shop.name.toLowerCase().includes(searchTerm.toLowerCase()) || shop.customer_name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const SetNewStatus = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const change = target.getAttribute("title") == "Deactive" ? "deactive" : "activate"
    try {
      const response = await axios.put(`https://pintu-sewa-backend.up.railway.app/api/admin/manage-shop/${change}/${shopId}`)
      ...
      <main className='flex-1 p-6'>
        ...
        <div className='overflow-x-auto bg-white shadow-md rounded-xl'>
          <table className='min-w-full text-sm text-left'>
            ...
            <tbody>
              {shops.map((shop) => (
                ...
                >
                  <td className='px-6 py-4'>{indexOfFirst + 1 + shops.indexOf(shop)}</td>
                  ...
                  <td className='px-6 py-4'>{shop.shop_status === "ACTIVE" ? <span className='text-green-600 font-medium'>Active</span> : <span className='text-red-500 font-medium'>Inactive</span>}</td>
          ...
          <div className='flex flex-wrap items-center justify-between px-6 py-4 text-sm text-gray-600'>
            <div>
              Menampilkan {indexOfFirst + 1} - {indexOfLast} dari {totalItems} shop
            </div>
            <div className='flex space-x-1 mt-2 sm:mt-0'>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md border ${num === currentPage ? "bg-blue-500 text-white" : "hover:bg-blue-100"}`}
                >
                  {num}
                </button>
              ))}
